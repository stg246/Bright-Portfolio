      ...
      <section className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
          <div className="lg:col-span-2 space-y-6">
            {[
              { icon: "fa-solid fa-phone", label: "Phone", value: siteConfig.phone, href: `tel:${siteConfig.phone}` },
              { icon: "fa-solid fa-envelope", label: "Email", value: siteConfig.email, href: `mailto:${siteConfig.email}` },
              { icon: "fa-solid fa-location-dot", label: "Location", value: siteConfig.location, href: null },
            ].map((item, i) => (
              <motion.div key={i} initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.5 }} className="card-base rounded-2xl p-6 flex items-start gap-4">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#d4a843] to-[#b8941f] flex items-center justify-center text-[#1a1a1a] shadow-lg shadow-[#d4a843]/20 shrink-0">
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7, delay: 0.2 }} className="lg:col-span-3">
            <form onSubmit={handleSubmit} className="card-base rounded-2xl p-8 gradient-border space-y-6">
              ...
              <button type="submit" disabled={submitted} className="w-full flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl bg-gradient-to-r from-[#d4a843] to-[#b8941f] text-[#1a1a1a] text-sm font-semibold shadow-lg shadow-[#d4a843]/25 hover:shadow-[#d4a843]/40 hover:-translate-y-0.5 transition-all duration-300 disabled:opacity-50 cursor-pointer">
                {submitted ? <><i className="fa-solid fa-circle-check" /> Message Sent!</> : <><i className="fa-solid fa-paper-plane" /> Send Message</>}
              </button>
            </form>
          </motion.div>
        </div>
